...
%
% [Q,ERR] = robot.ikconnorot(T) as above but also returns ERR which is the
% scalar final value of the objective function.
%
% [Q,ERR,EXITFLAG] = robot.ikconnorot(T) as above but also returns the
% status EXITFLAG from fmincon.
%
% [Q,ERR,EXITFLAG] = robot.ikconnorot(T, Q0) as above but specify the
% initial joint coordinates Q0 used for the minimisation.
%
% [Q,ERR,EXITFLAG] = robot.ikconnorot(T, Q0, options) as above but specify the
% options for fmincon to use.
%
% Trajectory operation::
%
% In all cases if T is 4x4xM it is taken as a homogeneous transform
% sequence and R.ikconnorot() returns the joint coordinates corresponding to
...
function [qstar, error, exitflag] = ikconnorotnorot(robot, T, q0, options)
    
    % check if Optimization Toolbox exists, we need it
    if ~exist('fmincon')
        error('rtb:ikconnorot:nosupport', 'Optimization Toolbox required');
    end
    ...
    problem.options = optimoptions('fmincon', ...
        ...
        'Display', 'off'); % default options for ikconnorot
    ...
    for t = 1: T_sz
        problem.objective = ...
            @(x) sumsqr(translDiff(T(:,:,t), robot.fkine(x)));
...
function res = translDiff(T, fk)
    res = T(1:3,4) - fk(1:3,4);
end
